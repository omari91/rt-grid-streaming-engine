...
\title{Operating State, Not Event Magnitude, Predicts Voltage-Limit Violations in Grid Redispatch Audit}
...
Recall alone understates the trade-off: of the 4,055 AC validations the selector triggers, only 239 find a violation -- precision of $\mathbf{5.9\%}$, a false-positive rate of $20.0\%$, and a false-discovery rate of $94.1\%$ (Table~\ref{tab:recall}). Roughly 19 in 20 selective validations this screen triggers come back clean.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Severity} & \textbf{FN rate} & \textbf{Precision} & \textbf{Catch \%} & \textbf{95\% CI (\%)} \\ \midrule
...
\begin{figure}[htbp]
    ...
    \caption{Per-event cycle time ($N=20{,}586$, log scale, JIT warm-up excluded) against the 20\,ms software deadline. The critical-path band sits almost entirely below it.}
