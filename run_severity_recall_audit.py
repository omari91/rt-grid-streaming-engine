"""
Real recall audit under a realistically-shaped, transparently-scaled
operating state (severity 1.75x on real SimBench shape, per
run_severity_sweep.py's onset analysis).

This mirrors the E2 recall-audit and out-of-sample regression methodology
exactly, applied fresh at this severity level:
1. Critical-path violations come directly from the streaming pipeline
   (3,910 critical events, all NR-solved; no new circularity risk since
   screening is magnitude-only and independent of load severity).
2. By default, the entire 16,676-event non-critical population is audited
   (full census, zero sampling uncertainty in the false-negative rate); a
   smaller sample_size can be passed for a quicker exploratory run.
3. The linear voltage-sensitivity regression is refit from scratch on a
   70/30 split of THIS severity's critical-path events (never touching the
   recall sample), then applied out-of-sample to the recall sample -- same
   discipline that caught the original 1.20-threshold circularity.

Usage
-----
    python run_severity_recall_audit.py
"""
import sys
import numpy as np
import pandas as pd
...
SEVERITY = float(sys.argv[1]) if len(sys.argv) > 1 else 1.75
SAMPLE_SIZE = int(sys.argv[2]) if len(sys.argv) > 2 else 16676  # full non-critical population (census); see docstring
...
def main():
    ...
    print(f"Severity: {SEVERITY}x documented network peak (real SimBench shape)")
    ...
    # 2. Real recall audit (sampled, non-critical events)
    ...
    # 3. Refit regression fresh on THIS severity's critical-path data (70/30 split)
    solved = solved.copy()
    ...
    print(f"  Caught: {n_caught}/{n_sample_violations} "
          f"({p_hat*100:.1f}%, Wilson 95% CI [{ci_low*100:.1f}%, {ci_high*100:.1f}%])" if n_sample_violations else "  No violations in this sample")
    ...
    out = {
        ...
        "sample_size": int(recall_row["sample_size"]),
        "sample_violations_found": n_sample_violations,
        "estimated_fn_rate": recall_row["estimated_fn_rate"],
        "estimated_fn_rate_ci95_low": recall_row["estimated_fn_rate_ci95_low"],
        "estimated_fn_rate_ci95_high": recall_row["estimated_fn_rate_ci95_high"],
    ...
    out_path = os.path.join(OUTPUT_DIR, f"severity_recall_audit_{SEVERITY:.2f}x{suffix}.csv")
